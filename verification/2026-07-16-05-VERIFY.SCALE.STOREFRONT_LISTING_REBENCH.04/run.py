#!/usr/bin/env python3
"""Gate for VERIFY.SCALE.STOREFRONT_LISTING_REBENCH.04."""
...
def main() -> int:
    ...
    gates = {
        ...
        "category_p95_after_di_fix": p95_ms <= P95_THRESHOLD_MS,
        "aggregation_trim_still_active": p95_ms <= WAVE1_BASELINE_MS + 30,
    }
    metrics = {
        "p95_ms": round(p95_ms, 2),
        "p50_ms": round(p50_ms, 2),
        "wave1_p95_ms": WAVE1_BASELINE_MS,
        "threshold_ms": P95_THRESHOLD_MS,
        "request_count": len(samples),
        "product_count": product_count,
        "category_path": category["path"],
        "note": "Bench category CMS has no product-listing element; confirms DI fix baseline",
        "errors": errors[:10],
    }

    ok = all(gates.values())
    report = {
        "claim_id": "VERIFY.SCALE.STOREFRONT_LISTING_REBENCH.04",
    ...
    print(f"VERIFY.SCALE.STOREFRONT_LISTING_REBENCH.04: {'PASS' if ok else 'FAIL'}")
